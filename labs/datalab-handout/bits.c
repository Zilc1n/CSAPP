...
int conditional(int x, int y, int z) {
    int sign = !!(x ^ 0);
    return (y & (sign << 31 >> 31)) | (z & ((!sign) << 31 >> 31));
}
/*
...
// diff = x - y; (1 0) | (($1 ^ $2) & (!!(diff ^ 0) | (diff >> 31 ^ 1)))
int isLessOrEqual(int x, int y) {
    int diff = x + (~y + 1);
    int sign_x = (x >> 31) & 1;
    int sign_y = (y >> 31) & 1;

    int cond1 = (sign_x & !sign_y);
    int cond2 = (!(sign_x ^ sign_y)) & (((diff >> 31) & 1) | !(diff ^ 0));
...
int logicalNeg(int x) {
    int sign = (((~x + 1) ^ x) >> 31 & 1) ^ 1;
    return sign & ((((x + 1) >> 31) & 1) ^ 1);
}
